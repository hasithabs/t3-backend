...
router.get('/', function(req, res, next) {
  BusRoute.find(function (err, results) {
...
router.get('/:id', function(req, res, next) {
  BusRoute.find({id: req.params.id}, function (err, results) {
    if (err) {
...
router.post('/', function(req, res, next) {
  BusRoute.findOne().sort({id:-1}).exec(function (err, resultMaxId) {
    ...
    BusRoute.create(req.body, function (err, results) {
      if (err) {
...
router.put('/:id', function(req, res, next) {
  BusRoute.findOneAndUpdate({id: req.params.id}, req.body, function (err, results) {
    if (err) {
      res.status(400).json({status: 400, error: "invalid id - " + req.params.id});
      return next(err);
    }
